...
// On s'est aider de l'IA ici pour bien construite la simulation avant d'utiliser nos algorithme MNA
...
export function computeCircuitCurrents(
  ...
  for (let i = 0; i < queue.length; i++) {
    const wireA = wireById.get(queue[i]);
    if (!wireA) continue;
    ...
    for (const wireB of circuit.wires) {
      ...
      let pIsLastA = false,
        pIsFirstB = false,
        found = false;
      if (dist(lastA, firstB) < GRID * 0.6) {
        pIsLastA = true;
        pIsFirstB = true;
        found = true;
      } else if (dist(lastA, lastB) < GRID * 0.6) {
        pIsLastA = true;
        pIsFirstB = false;
        found = true;
      } else if (dist(firstA, firstB) < GRID * 0.6) {
        pIsLastA = false;
        pIsFirstB = true;
        found = true;
      } else if (dist(firstA, lastB) < GRID * 0.6) {
        pIsLastA = false;
        pIsFirstB = false;
        found = true;
      }

...
const PROP_SCHEMAS: Record<ComponentType, ComponentPropertySchema> = {
  ...
  vsource: {
    voltage: { label: "Voltage (V)", type: "number", default: 5, step: 0.5 },
  },
  ground: {},
...
const COMPONENT_DEFS: Record<ComponentType, ComponentDef> = {
  ...
  capacitor: {
    ...
    propDefs: [
      { key: "capacitance", label: "Capacitance (F)", type: "number", min: 0 },
    ],
    draw(ctx, comp, sel, hov) {
      const gap = 7,
  ...
  vsource: {
    ...
    propDefs: [
      { key: "voltage", label: "Voltage (V)", type: "number", step: 0.5 },
    ],
    draw(ctx, comp, sel, hov) {
      const r = GRID * 0.85,
...
export function generateNetlist(circuit: Circuit): Netlist {
  ...
  if (groundRoots.size === 0)
    warnings.push("No ground component found. Node '0' will not be defined.");
  ...
  for (const [node, count] of nodeCount) {
    if (count < 2)
      warnings.push(`Node ${node} appears to be floating (only 1 connection).`);
  }
      ...
      >
        ...
        >
          ...
          {netlist.warnings.length > 0 && (
            ...
            >
              {netlist.warnings.length} warning
              {netlist.warnings.length > 1 ? "s" : ""}
            </span>
          )}
          <button
            onClick={copy}
...
function Toolbar({ state, dispatch, cam, onShowNetlist }: ToolbarProps) {
  ...
  const importCircuit = () => {
    ...
    inp.onchange = (e: Event) => {
      ...
      r.onload = (ev) => {
        try {
          const d = JSON.parse(ev.target?.result as string) as Circuit;
          dispatch({
            type: "LOAD",
  ...
  return (
    ...
    >
      ...
      <div style={sep} />
      <button style={btn} onClick={exportCircuit}>
        ↓ Export
      </button>
      <button style={btn} onClick={importCircuit}>
        ↑ Import
      </button>
      <div style={sep} />
      <button
...
function StatusBar({ state }: { state: AppState }) {
  ...
  const tips: Record<ToolMode, string> = {
    select:
      "Click to select · Shift+click / drag-box multi-select · R rotate · Del delete · Ctrl+Z/Y undo/redo",
  ...
  return (
    ...
    >
      ...
      <span>
        {state.components.length} comp · {state.wires.length} wires
      </span>
    </div>
  );
}
...
export default function App() {
  ...
  // update project when components or wires change
  useEffect(() => {
          ...
          {empty && (
            ...
            >
              ...
              >
                ...
                <br />
                then click on the canvas to place it
              </div>
            </div>
          )}
